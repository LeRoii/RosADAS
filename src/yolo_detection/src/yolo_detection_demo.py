...
class YOLO_detection:
    def __init__(self):
        self.image_pub = rospy.Publisher("YOLO_detect_demo_result", Image)
        ...
        self.num_classes = 80
        self.classes = load_classes("/home/iairiv/ros_pkgs/src/yolo_detection/src/data/coco.names")
        self.colors = random_color()
        self.cfg_file = "/home/iairiv/ros_pkgs/src/yolo_detection/src/cfg/yolov3.cfg"
        self.weights_file = "/home/iairiv/ros_pkgs/src/yolo_detection/src/yolov3.weights"
    ...
    def write(self, output, img):
        ...
        for x in output:
            ...
            t_size = cv2.getTextSize(label, cv2.FONT_HERSHEY_PLAIN, 4, 4)[0]
            c1_text = c1[0], c1[1] - t_size[1] - 4
            c2_text = c1[0] + t_size[0] + 3, c1[1]
            cv2.rectangle(img, c1_text, c2_text, color, -1)
            cv2.putText(img, label, (c1_text[0], c2_text[1]), cv2.FONT_HERSHEY_PLAIN, 4, [225, 255, 255], 4)
        return img
...
def random_color():
    colors = {0:[0, 0, 255], 1:[221,160,221], 2:[255, 191, 0], 3:[47,62,139], 5:[255, 131, 111], 7:[139,60,71], 80:[205,0,205]}
    # for i in range(81):
