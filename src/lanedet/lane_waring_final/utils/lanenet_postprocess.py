...
class _LaneNetCluster(object):
    ...
    def _embedding_feats_dbscan_cluster(embedding_image_feats):
        ...
        :return:
        """
        db = DBSCAN(eps=CFG.POSTPROCESS.DBSCAN_EPS, min_samples=CFG.POSTPROCESS.DBSCAN_MIN_SAMPLES, n_jobs=-1)
        try:
...
class LaneNetPostProcessor(object):
    ...
    """
    def __init__(self, ipm_remap_file_path='/space/code/roslane/src/lanedet/lane_waring_final/data/tusimple_ipm_remap.yml'):
        """
    ...
    def postprocess(self, binary_seg_result, instance_seg_result=None,
        ...
        fit_params = []
        detectedLanes = []
        lanePointsY = np.linspace(CFG.LANE_START_Y, CFG.LANE_END_Y, 12)
        ...
        for lane_index, coords in enumerate(lane_coords):
            ...
            lanePointsX = fit_param[0] * lanePointsY ** 2 + fit_param[1] * lanePointsY + fit_param[2]
        ...
        return ret
            ...
            # plot_y = np.linspace(700, 945, 12)
            # fit_x = fit_param[0] * plot_y ** 2 + fit_param[1] * plot_y + fit_param[2]

            # for idx in range(len(plot_y)):
            #     cv2.circle(source_image, (int(fit_x[idx]),int(plot_y[idx])), 5, (255,0,0), -1) #画成小圆点
        ...
        # cv2.imwrite('polyimg.png',source_image)

        

        # tusimple test data sample point along y axis every 10 pixels
        # source_image_width = 1920#source_image.shape[1]
        # IAIR_line_points = []
        ...
        #         points.append([interpolation_src_pt_x, interpolation_src_pt_y])
        #     IAIR_line_points.append(points)
